...
2. Dozvedieť sa o rôznych prvkoch, z ktorých sa licencie skladajú, ako je uvedenie autora (attribution), \(ne-\)komerčné použitie (\(non-\)commercial), odvodený obsah (derivatives) atď.  
...
1. Vedieť použiť existujúce zdroje na výber vhodnej licencie na písanú výskumnú prácu, na základe požadovanej slobody poskytnutej iným používateľom na použitie/opätovné použitie diela alebo na základe jej obmedzenia.  Will be able to use existing resources to choose an appropriate license for written research work, based on the desired freedom/limitation for others to use/reuse.
